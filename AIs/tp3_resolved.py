# With this template, we are building an AI that will apply 
# combinatorial game theory tools against a greedy opponent.

# Unless you know what you are doing, you should use this 
# template with a very limited number of pieces of cheese, 
# as it is very demanding in terms of computations.

# The first thing you should do is copy this file (tp3.py) to 
# the AIs folder of pyrat. For example:
# cp ./ai/tp3/tp3.py ./PyRat/AIs/

# A typical use would be:
# python pyrat.py -d 0 -md 0 -p 7 --rat AIs/tp3.py --python AIs/manh.py --nonsymmetric --tests 100 --nodrawing --synchronous

# If enough computation time is allowed, it is reasonable to 
# grow the number of pieces of cheese up to around 15. For example:
# python pyrat.py -d 0 -md 0 -p 15 --rat AIs/tp3.py --python AIs/manh.py --nonsymmetric --tests 100 --nodrawing --synchronous
...
# The first thing we do is programming the AI of the opponent, so that 
# we know exactly what will be its decision in a given situation.
def distance(la, lb):
    ax, ay = la
    bx, by = lb
...
# With this template, we are building an AI that will apply combinatorial 
# game theory tools against a greedy opponent.
...
# It takes as arguments a given situation, and return a best target piece of 
# cheese for the player, such that aiming to grab this piece of cheese will 
# eventually lead to a maximum score. It also returns the corresponding score.
def best_target(playerLocation, opponentLocation, playerScore, opponentScore, piecesOfCheese):
    # First we should check how many pieces of cheese each player has, to see if the play is over.
    # It is the case if no pieces of cheese are left, or if playerScore or opponentScore is more 
    # than half the total number playerScore + opponentScore + piecesOfCheese
...
# Move the agent in the labyrinth in the direction of the target.
# Use the function move and the different directions defined at the beginning of this file.
...
# If both players are on the same square and there is a cheese on the square,
# each player gets 0.5 point.
...
def simulate_game_until_target(target, playerLocation, opponentLocation, playerScore, opponentScore, piecesOfCheese):   
    # While the target cheese has not yet been eaten by one of the player,
    # we simulate how the game will evolve.   
    while target in piecesOfCheese:
        ...
        # Every time that we move the opponent also moves.
        # Update the position of the opponent using turn_of_opponent and move.
...
# During our turn we continue going to the next target, unless the piece of cheese it 
# originally contained has been taken.
